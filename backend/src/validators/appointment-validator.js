...
/**
 * Used when booking as a guest (no auth required)
 */
export const guestAppointmentSchema = Joi.object({
  timeslotId: Joi.string().uuid().required().messages({
    "string.uuid": "Time slot ID must be a valid UUID",
    "any.required": "Time slot ID is required"
  }),
  ...
  guest_name: Joi.string().min(2).max(255).required().messages({
    "string.min": "Name must be at least 2 characters",
    "string.max": "Name must be less than 255 characters",
    "any.required": "Name is required"
  }),
  guest_email: Joi.string().email().required().messages({
